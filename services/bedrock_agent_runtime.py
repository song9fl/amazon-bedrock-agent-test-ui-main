...
def invoke_agent(agent_id, agent_alias_id, session_id, prompt):
    try:
        ...
        # Extract the main response text from the 'result' field
        output_text = response.get("result", "")
        citations = []
        trace = {}

        # Check if 'result' was properly retrieved
        if not output_text:
            output_text = "No response was generated by the chatbot."

        # Optional: Remove inline placeholders like %[1]% or [1] only if they exist
        ...
        for event in response.get("completion", []):
            ...
            if chunk and "attribution" in chunk:
                # Collect citations from attribution without modifying output_text
                citations.extend(chunk["attribution"]["citations"])

            # Collect trace information if needed
